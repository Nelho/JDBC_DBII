...
public class ClienteDAO extends DataBaseDAO{
	...
	public void createCliente(Cliente cliente) throws SQLException{
		...
		String INSERT_SQL = "INSERT INTO public.clientes(id, nome, email, cpf, nascimento, endereco) VALUES (?, ?, ?, ?, ?, ?)";
	...
	public Cliente retriveCliente(int codigo) throws SQLException{
		Cliente cliente = null;
		if(codigo <= 0){
			throw new SQLException("Código menor ou igual a zero!");
		}
		String SELECT_SQL = "select NOME,EMAIL,CPF,NASCIMENTO,ENDERECO FROM CLIENTES WHERE id = ?";
		...
		try{
			...
			if(rs.next()){
				cliente = new Cliente(codigo,rs.getString("NOME"),rs.getString("EMAIL"),rs.getString("CPF")
						,rs.getDate("NASCIMENTO"),rs.getString("ENDERECO"));
	...
	public void updateCliente(Cliente cliente) throws SQLException{
		...
		String UPDATE_SQL = "UPDATE clientes SET nome=?, email=?, cpf=?, nascimento=?, endereco=? WHERE id=?";
		PreparedStatement pstmt = null;
		try{
